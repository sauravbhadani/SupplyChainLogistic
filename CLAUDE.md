...
- `/ArchitectureReview` - Pre-implementation architecture design/review gate (run after `/PRDFeasibility`, before `/ImplementFeature`). `design` mode produces a component/data-model/API design plus a system-level HLD (Mermaid diagram); `mode="lld"` produces per-module class/sequence diagrams grounded in real code when it exists; `mode="conformance"` checks a built feature against its approved design
